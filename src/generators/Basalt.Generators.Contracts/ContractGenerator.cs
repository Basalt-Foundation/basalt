...
public sealed class ContractGenerator : IIncrementalGenerator
{
    ...
    // C-01: Diagnostic for selector collision detection
    private static readonly DiagnosticDescriptor SelectorCollision = new(
        "BSLT001",
        "FNV-1a selector collision",
        "Selector collision: '{0}' and '{1}' both map to 0x{2:X8}",
        "Basalt.Contracts",
        DiagnosticSeverity.Error,
        isEnabledByDefault: true);

    // C-02: Diagnostic for non-ASCII method names
    private static readonly DiagnosticDescriptor NonAsciiMethodName = new(
        "BSLT002",
        "Non-ASCII method name in contract dispatch",
        "Method '{0}' contains non-ASCII characters which may cause selector collisions due to byte truncation",
        "Basalt.Contracts",
        DiagnosticSeverity.Error,
        isEnabledByDefault: true);
    ...
    {
        // C-02: Check for non-ASCII method names
