...
| BST010 | Warning | Storage write before policy enforcement | `.Set()` call appears before `EnforceTransfer()` or `EnforceNftTransfer()` in the same method. Storage mutations should follow policy checks (checks-effects-interactions). |
...
| BST012 | Warning | Missing policy enforcement in transfer override | A class deriving from a BST token type overrides `TransferInternal` without calling `EnforceTransfer` or `EnforceNftTransfer`. Policy hooks will be bypassed. |
